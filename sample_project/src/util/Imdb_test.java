...
public class Imdb_test {
	...
	public static void main(String[] args) {
		...
		DefaultSelenium selenium = new DefaultSelenium("localhost", 4444, "*chrome", "http://www.imdb.com/");
		selenium.start();
		selenium.setTimeout("220000");
		for(Double i=1.0000075; i<=1.9999999; i+=0.0000001){
			
			String strSub= i.toString().substring(2,9);
			String url = "http://www.imdb.com/title/tt"+strSub+"/";
			...
			try{
				...
				if(response==200){
					selenium.open(url);
		    		String s=selenium.getText("overview-top");
